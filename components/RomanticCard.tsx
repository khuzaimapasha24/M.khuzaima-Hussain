...
const BirthdayCard: React.FC<BirthdayCardProps> = ({ data }) => {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      link.download = `BirthdayCard-${data.recipientName}.png`;
  ...
  const getVibeStyles = () => {
    switch (data.vibe) {
      case BirthdayVibe.GLITCH:
        return "bg-black border-cyan-400 text-cyan-400 shadow-[0_0_50px_rgba(34,211,238,0.2)]";
      case BirthdayVibe.ROYAL:
        return "bg-gradient-to-br from-amber-50 to-yellow-100 border-yellow-500 text-amber-900";
      case BirthdayVibe.PASTEL:
        return "bg-gradient-to-br from-pink-50 to-blue-50 border-pink-200 text-pink-900";
      case BirthdayVibe.PARTY:
      default:
        return "bg-gradient-to-br from-purple-500 to-pink-500 border-white text-white";
    }
  };
  ...
  return (
    <div className="flex flex-col items-center gap-10 w-full animate-in zoom-in-95 duration-1000">
      <div className="perspective-1000 w-[340px] h-[520px] sm:w-[420px] sm:h-[600px] cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
        <div className={`card-inner relative w-full h-full preserve-3d shadow-[0_20px_80px_rgba(0,0,0,0.5)] rounded-[3rem] transition-all duration-1000 ${isFlipped ? 'flipped' : ''}`}>
          ...
          <div ref={!isFlipped ? cardRef : null} className={`backface-hidden absolute inset-0 w-full h-full rounded-[3rem] border-[14px] ${getVibeStyles()} p-12 flex flex-col items-center justify-between text-center overflow-hidden`}>
             ...
             <div className="z-10 mb-8 font-serif italic opacity-60 text-sm">
                "Flip to see your special gift"
             </div>
          </div>
          ...
          <div ref={isFlipped ? cardRef : null} className={`backface-hidden absolute inset-0 w-full h-full rounded-[3rem] border-[14px] ${getVibeStyles()} rotate-y-180 p-10 flex flex-col items-center overflow-y-auto scrollbar-hide`}>
            ...
            <div className="text-center space-y-8 px-2 flex-grow flex flex-col justify-center">
              ...
              <div className="pt-8 border-t border-current/20">
                 <p className="text-[10px] uppercase tracking-[0.3em] opacity-60 mb-2">From the bottom of my heart,</p>
      ...
      <div className="flex gap-4">
        <button onClick={() => setIsFlipped(!isFlipped)} className="px-10 py-4 bg-white/10 hover:bg-white/20 text-white rounded-2xl border border-white/20 backdrop-blur-xl transition-all font-bold shadow-xl">
          {isFlipped ? 'Show Front' : 'Read Message'}
        </button>
        <button disabled={isCapturing} onClick={handleDownload} className="px-10 py-4 bg-gradient-to-r from-pink-500 to-rose-600 text-white rounded-2xl shadow-2xl transition-all font-black uppercase tracking-widest text-xs flex items-center gap-3">
          {isCapturing ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : 'Download Card'}
        </button>
      </div>
    </div>
  );
